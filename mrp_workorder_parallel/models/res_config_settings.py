...
class ResConfigSettings(models.TransientModel):
    ...
    repair_location_id = fields.Many2one(
        ...
        config_parameter='mrp_workorder_parallel.repair_location_id',
        domain=[('usage', '=', 'internal')],
    )
    repair_workcenter_id = fields.Many2one(
        'mrp.workcenter',
        string='Repair Workcenter',
        config_parameter='mrp_workorder_parallel.repair_workcenter_id',
    )
    repair_loss_id = fields.Many2one(
        'mrp.workcenter.productivity.loss',
        string='Repair Blocking Loss',
        config_parameter='mrp_workorder_parallel.repair_loss_id',
