...
class MgmtStatementWizard(models.TransientModel):
    ...
    parallel_production_id = fields.Many2one(
        related="parallel_workorder_id.production_id", store=True
    )
    sequential_production_id = fields.Many2one("mrp.production")

    scanned_serial = fields.Char("Serial", required=False)

    nonconformity_id = fields.Many2one("mgmt.nonconformity", required=True)
    nonconformity_id_domain = fields.Char(compute="_compute_nonconformity_id_domain")
    ...
    component_id_domain = fields.Char(compute="_compute_component_id_domain")
    ...
    statement_ids = fields.One2many(
        "mgmt.statement", compute="_compute_statement_ids", string="Existing Statements"
    )
    ...
    def _compute_nonconformity_id_domain(self):
        ...
        for wizard in self:
            nonconformity_ids = (
                self.env["mgmt.nonconformity"].search([("nc_type", "=", nc_type)]).ids
            )
            if nonconformity_ids:
                wizard.nonconformity_id_domain = str([("id", "in", nonconformity_ids)])
            else:
    ...
    def default_get(self, fields):
        ...
        if workorder_id:
            ...
            if wo and wo.production_id.bom_id:
                component_ids = wo.production_id.bom_id.bom_line_ids.mapped(
                    "product_id"
                ).ids
            ...
            if nc_type:
                nonconformity_ids = (
                    self.env["mgmt.nonconformity"]
                    .search([("nc_type", "=", nc_type)])
                    .ids
                )
    ...
    def _onchange_scanned_serial(self):
        ...
        if not seq_prod:
            raise UserError(
                _("This serial number does not belong to any production order.")
            )
    ...
    def action_create(self):
        ...
        if self._context.get("save_and_new"):
            new_context = {
                "default_parallel_workorder_id": self._context.get(
                    "default_parallel_workorder_id"
                ),
