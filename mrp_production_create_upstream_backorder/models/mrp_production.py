...
class MrpProduction(models.Model):
    ...
    def _create_upstream_backorder(self, factor):
        ...
        self.ensure_one()
        move_orig_ids = self.move_raw_ids.move_orig_ids.filtered(
        ...
        if move_orig_ids and factor > 0:
            ...
            for product_id in move_orig_ids.mapped("product_id"):
                ...
                # Check that no open product moves exist
                open_product_moves = self.move_raw_ids.move_orig_ids.filtered(
                    lambda m: m.state not in ("done", "cancel")
                    and m.product_id == product_id
                )
                if not open_product_moves:

                    # Calculate new qty
                    product_moves = move_orig_ids.filtered(
                ...
                move_lines.append(
                    (
                        ...
                        0,
                        {
                            ...
                            "move_dest_ids": [(6, 0, move_raw_ids.ids)],
