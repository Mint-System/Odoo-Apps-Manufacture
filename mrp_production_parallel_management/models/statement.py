...
class MgmtStatement(models.Model):
    ...
    parallel_production_id = fields.Many2one(
        "mrp.production", compute="_compute_parallel_production_id", store=True
    )
    lot_id = fields.Many2one(
        "stock.lot", string="Serial/Lot", compute="_compute_lot_id", store=True
    )
    user_id = fields.Many2one(
        "res.users", default=lambda self: self.env.user, string="Reported by"
    )
    description = fields.Char()

    # components of the bom
    component_id = fields.Many2one(
        "product.product", string="Component (if applicable)"
    )
    ...
    def _compute_parallel_production_id(self):
        for rec in self:
            _logger.warning(
                f"###### rec.production_id.type: {rec.production_id.type}, rec.workorder_id.production_id.type: {rec.workorder_id.production_id.type}"
            )
            if (
                not rec.production_id.type == "parallel"
                and not rec.workorder_id.production_id.type == "parallel"
            ):
                continue

            _logger.warning(
                f"######### par pro id: {rec.workorder_id.production_id.id}"
            )
    ...
    def _compute_bom_component_ids(self):
        for rec in self:
            rec.bom_component_ids = rec.production_id.bom_id.bom_line_ids.mapped(
                "product_id"
            )
