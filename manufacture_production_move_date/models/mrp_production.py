...
class MrpProduction(models.Model):
    ...
    # @api.depends("date_start", "product_id")
    # def _compute_date_finished(self):
    #     """
    #     OVERWRITE core compute: only compute date_finished,
    #     do NOT propagate onto move_raw_ids / move_finished_ids —
    #     that side is fully owned by date_move / _set_move_dates().
    #     """
    #     for production in self:
    #         if production.date_start:
    #             date_finished = production.date_start + relativedelta(
    #                 days=production.product_id.produce_delay
    #             )
    #             date_finished += relativedelta(
    #                 days=production.company_id.manufacturing_lead
    #             )
    #             if date_finished == production.date_start:
    #                 date_finished += relativedelta(hours=1)
    ...
    #                 (1, m.id, {"date_deadline": production.date_planned_start})
